...
3. 로컬 파일은 **Play local file…** 버튼으로 선택기를 열어 바로 재생
   (목록에는 저장되지 않는 일회성 재생).
4. 상단 **Playback settings**에서 "Always on top"과 창 크기(Small/Medium/Large)를 조절.
...
- `src/app.rs` — egui GUI (저장 목록, 폼, 로컬 파일 선택기)
- `src/main.rs` — 진입점
